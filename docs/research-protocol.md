...
- `Δt_renorm = 1.0` time unit: chosen to be a small fraction of the shortest special-solution
  ...
  sweep's total cost.
- `T_max = 60.0` time units for the perturbation-magnitude sweep (roughly 9–13 periods of the
  three special solutions), `T_max = 40.0` for the mass-ratio sweep (chosen shorter only to
  keep the larger mass-ratio grid computationally tractable; this asymmetry is disclosed, not
  hidden, and the report treats the two sweeps' absolute exponent values as not directly
  comparable across sweeps for this reason — only within-sweep comparisons are made).
